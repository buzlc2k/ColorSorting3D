    ...
    public class RecordComparer<T> : IComparer<T> where T : class
    {
        ...
        public int Compare(T value, T target)
        {
            ...
            for (int i = 0; i < keyInfos.Count; i++)
            {
                ...
                CheckNull(expectedValue, value.ToString(), keyInfos[i].FieldInfo.Name);
                CheckNull(actualValue, target.ToString(), keyInfos[i].FieldInfo.Name);
            ...
            static void CheckNull(object obj, string objName, string fieldName)
            {
                if (obj == null)
                    throw new NullReferenceException($"{objName} doesn't have {fieldName} value");
            }
        }
